...
(*let xyCut img =
  ...
  let q = Queue.create ()
  and vertically = ref false in
    Queue.add box q;
    for i = 0 to 2 do
      let length = Queue.length q in
      for i = 0 to length - 1 do
        let q2 = transfer (exec img !vertically (Queue.take q)) in
        while (Queue.length q2) > 0 do
          Queue.add (Queue.take q2) q
        done;
      done;
...
let cut img =
  ...
  and display = Sdlvideo.set_video_mode w h [`DOUBLEBUF] in
   let box = {
      pos = (0,0);
      width = w;
      height = h
